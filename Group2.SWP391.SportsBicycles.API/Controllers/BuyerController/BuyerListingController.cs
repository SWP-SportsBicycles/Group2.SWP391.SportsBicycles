...
namespace Group2.SWP391.SportsBicycles.API.Controllers.BuyerController
{
    ...
    public class BuyerListingController : ControllerBase
    {
        ...
        // ================= WISHLIST =================
        [HttpPost("wishlist/{bikeId}")]
        public async Task<IActionResult> AddToWishlist(Guid bikeId)
        {
            var result = await _service.AddToWishlistAsync(bikeId);
            return HandleResult(result);
        }

        [HttpDelete("wishlist/{bikeId}")]
        public async Task<IActionResult> RemoveFromWishlist(Guid bikeId)
        {
